...
def detect_rgb_edges(input_rgb_img,
    ...
    for tg_pix in theta_grad_pix:
        ...
        if  abs(theta) <= theta_thresh and (gmagn >= magn_thresh * (1 / 4) and gmagn <= magn_thresh * (2 / 4)):
            output_img.putpixel(coords, 64)
            lowThreshCount += 1;
        elif abs(theta) <= theta_thresh and (gmagn >= magn_thresh * (2 / 4) and gmagn <= magn_thresh * (3 / 4)):
            output_img.putpixel(coords, 128)
            medThreshCount += 1;
        elif abs(theta) <= theta_thresh and (gmagn >= magn_thresh * (3 / 4) and gmagn <= magn_thresh * (4 / 4)):
            output_img.putpixel(coords, 196)
            highThreshCount += 1;
        elif abs(theta) <= theta_thresh and gmagn >= magn_thresh * (4 / 4):
            output_img.putpixel(coords, 255)
            fullThreshCount += 1;
        else:
            output_img.putpixel(coords, 0)

    edgePixelCount = lowThreshCount + medThreshCount + highThreshCount + fullThreshCount;
    blurrinessRatio = 1 - ((fullThreshCount * 1.0) / edgePixelCount);

    print ("Low: " + str(lowThreshCount) + " Med: " + str(medThreshCount) + 
        " High: " + str(highThreshCount) + " Full: " + str(fullThreshCount));
    print "Total Edges: " + str(edgePixelCount) + " Blurriness: " + str(blurrinessRatio);

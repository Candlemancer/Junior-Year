...
CandleStats::CandleStats(std::vector<double> values) :
	NUM_TESTS(0),
	functionName("Custom Times"), 
	times(values) {
...
void CandleStats::calculateMean() {
	...
	mean = std::accumulate(times.cbegin(), times.cend(), 0);
...
void CandleStats::calculateStandardDeviation() {
	...
	for (auto&& i : times) {
		differences.push_back( (i - mean) * (i - mean) );
	}
